...
export default function CreekMap() {
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      // Setup the "Magnetic" Click Logic
      map.current?.on('click', (e) => {
        if (!showCreeks) return; // Don't show popups if creeks are hidden
  ...
  }, []); // Remove showCreeks from here so the map doesn't re-initialize

  // --- THE BULLETPROOF TOGGLE LOGIC ---
  useEffect(() => {
    if (!map.current) return;

    const updateLayers = () => {
      // Instead of visibility: 'none', we use Opacity. This prevents the "Blank Map" crash.
