...
const TOWNS = [
  ...
  { 
    ...
    attractions: ["Assateague Lighthouse", "Wild Pony Viewing", "Misty of Chincoteague Museum"]
  },
  { 
    name: "Saxis", 
    coords: [-75.720, 37.923], 
    desc: "A true crabbing village with sunset views like no other.",
    attractions: ["Saxis Island Museum", "Wildlife Management Area", "Martha's Kitchen Seafood"]
  },
  { 
    name: "Parksley", 
    coords: [-75.649, 37.790], 
    desc: "A historic railroad town with beautiful Victorian architecture.",
    attractions: ["Railway Museum", "Historic Five-and-Dime", "Town Square Festivals"]
  },
  { 
    name: "Accomac", 
    coords: [-75.666, 37.722], 
    desc: "The historic county seat, filled with 18th-century architecture.",
    attractions: ["Ker Place Museum", "Historic Court Green", "Federal-style Mansions"]
  },
  { 
    name: "Onancock", 
    coords: [-75.743, 37.711], 
    desc: "A vibrant port town known for its arts scene and fine dining.",
    attractions: ["Tangier Ferry", "North Street Playhouse", "Historic Wharf District"]
  },
  { 
    name: "Wachapreague", 
    coords: [-75.688, 37.607], 
    desc: "The 'Little City by the Sea,' a premier destination for fishing.",
    attractions: ["Offshore Charter Fishing", "Island House Restaurant", "VIMS Marine Center"]
  },
  { 
    name: "Exmore", 
    coords: [-75.823, 37.531], 
    desc: "The bustling retail hub of the shore, perfect for shopping.",
    attractions: ["Blue Crow Antique Mall", "Exmore Town Park", "Central Shopping District"]
  },
  { 
    name: "Nassawadox", 
    coords: [-75.861, 37.472], 
    desc: "Quiet residential charm with deep ties to shore history.",
    attractions: ["Brownsville Preserve", "Holly Grove Vineyards", "Hiking & Birding Trails"]
  },
  { 
    name: "Eastville", 
    coords: [-75.945, 37.351], 
    desc: "Home to the oldest continuous court records in the country.",
    attractions: ["1632 Court Records", "Eyre Hall Gardens", "Historic Prison Site"]
  },
  { 
    name: "Cheriton", 
    coords: [-75.975, 37.288], 
    desc: "A quaint stop minutes from the bay, surrounded by sprawling farmland.",
    attractions: ["Local Artisan Shops", "Chatham Vineyards", "Proximity to Cape Charles"]
  },
  { 
    name: "Cape Charles", 
    coords: [-76.023, 37.267], 
    desc: "The southern jewel. Historic, walkable, and beachfront.",
    attractions: ["Public Beach & Pier", "Bay Creek Golf Club", "Boutique Main Street"]
  }
];
...
export default function TownTour() {
  ...
  // Helper to update labels reliably
  ...
  return (
    <main className="flex flex-col md:flex-row min-h-screen bg-slate-950 overflow-hidden">
      {/* BRANDING HIGHLIGHT */}
      <div className="fixed top-4 left-4 z-50 bg-blue-600/90 backdrop-blur px-4 py-2 rounded-full shadow-lg border border-blue-400/50">
        <span className="text-white font-black text-xs uppercase tracking-widest">Hill Realty Exclusive Tour</span>
      </div>
      ...
      <div className="w-full md:w-1/3 h-screen overflow-y-scroll snap-y snap-mandatory z-10 no-scrollbar md:order-1 relative mt-[55vh] md:mt-0 pb-[45vh]">
        {TOWNS.map((town) => (
          <section key={town.name} data-town={town.name} className="town-section h-[45vh] md:h-screen snap-center flex flex-col justify-start pt-12 px-8 md:px-12 bg-slate-950/90 backdrop-blur-sm md:bg-transparent" >
            ...
            <p className="text-slate-300 text-sm md:text-lg leading-relaxed max-w-md italic mb-6">"{town.desc}"</p>
            
            <div className="space-y-2">
              <span className="text-blue-500 font-bold text-[10px] uppercase tracking-widest">Main Attractions</span>
              <ul className="grid grid-cols-1 gap-2">
                {town.attractions.map(attr => (
                  <li key={attr} className="text-slate-400 text-xs flex items-center gap-2">
